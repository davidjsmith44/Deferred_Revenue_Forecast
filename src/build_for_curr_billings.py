...
df_hist = df_billings.groupby(['curr', 'period'], as_index=False).sum()
df_fcst = df_fcst.groupby(['curr', 'period'], as_index=False).sum()

def build_one_curr_history(this_curr, df_hist):
    this_curr_df = df_hist[df_hist['curr']==this_curr].copy()
    list_to_drop = ['curr', 'Period_Weeks', 'is_forecast']
    for col in this_curr_df.columns:
        if '_US' in col:
            list_to_drop.append(col)
    this_curr_df = this_curr_df[['period', 'deferred_3Y_DC', 'deferred_2Y_DC', 'deferred_1Y_DC',
...
df_hist_list = ['AUD_hist', 'EUR_hist', 'GBP_hist', 'JPY_hist']
df_fcst_list = ['AUD_fcst', 'EUR_fcst', 'GBP_fcst', 'JPY_fcst']

for i in range(len(curr_list)):
    df_hist_list[i] = build_one_curr_history(curr_list[i], df_hist)
    df_hist_list[i].set_index('period', inplace=True)

    df_fcst_list[i] = build_one_curr_history(curr_list[i], df_fcst)
    df_fcst_list[i].set_index('period', inplace=True)

print(df_hist_list[2])
...
with pd.ExcelWriter('../output/FOREIGN_CURR_BILLINGS.xlsx') as writer:
    for i in range(len(curr_list)):
        ...
        df_hist_list[i].to_excel(writer, sheet_name=curr_list[i], startrow=4, startcol=1, header=True)
